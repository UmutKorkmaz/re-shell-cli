...
export class VueTemplate extends BaseTemplate {
  ...
  private generateMainFile(): string {
    ...
    if (hasTypeScript) {
      return `import { createApp, App as VueApp } from 'vue';
...
window.${componentName} = {
  ...
  unmount: () => {
    if (window.${componentName}.app) {
      window.${componentName}.app.unmount();
...
if (import.meta.env.DEV) {
  ...
  <div class="${normalizedName}-app">
    <header class="${normalizedName}-header">
      <h1>${name}</h1>
      <p>A Vue microfrontend built with Re-Shell CLI</p>
    </header>
...
<body>
  ...
  private generateDockerfile(): string {
    return `# Multi-stage build for Vue microfrontend
